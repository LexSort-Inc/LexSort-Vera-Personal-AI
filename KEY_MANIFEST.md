...
|---------|-----------------------------|---------------|--------|
| Embedded public key | `3183e9e4a95b99b3` | `vera-freeware/src-tauri/lexsort_public_key.bin` | Active (v1.0.0+) |
...
2. Replace `vera-freeware/src-tauri/lexsort_public_key.bin` with the new public key file.
3. Rebuild the Tauri binary for all platforms (macOS, Windows, Linux).
4. Update the fingerprint in `vera-freeware/src-tauri/tests/contracts.rs` to the new key's first 8 bytes.
5. Publish new binaries.

## Verification
To verify the embedded public key fingerprint:
```bash
hexdump -n 8 -v -e '8/1 "%02x"' vera-freeware/src-tauri/lexsort_public_key.bin
```
