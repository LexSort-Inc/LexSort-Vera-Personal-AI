...
export function QuickOrganizer({ activeModel = "llama3.2:3b", serverPort = 11434 }: QuickOrganizerProps) {
    ...
    useEffect(() => {
        ...
        const handleTaskDue = (e: Event) => {
            ...
            const title = detail?.title || 'Task due';
            if ('Notification' in window && Notification.permission === 'granted') {
                new Notification('VERA — Task Due', { body: title });
            }
            loadTasks();
        };
    ...
    async function handleQuickAdd() {
        ...
        try {
            const task = await createTaskLocal({
                ...
                recurrenceRule: parsed.recurrence_rule,
            });
                        ...
                        <div className="settings-body" style={{ display: 'flex', flexDirection: 'column', gap: '12px', padding: '16px' }}>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="qo-add-input"
                                    style={{ width: '100%' }}
                                    placeholder="e.g. every Monday, daily, every 3 days"
                                    value={editingTask.recurrence_rule || ''}
                                    onChange={e => setEditingTask({ ...editingTask, recurrence_rule: e.target.value || null })}
                                />
                            </div>
                                    ...
                                    onClick={() => {
                                        ...
                                        if (editingTask.id) {
                                            ...
                                            createTaskLocal({
                                                ...
                                                recurrenceRule: editingTask.recurrence_rule || null,
                                            }).then(task => {
                                                setTasks(prev => [...prev, task]);
                                                setShowEditModal(false);
                                            });
